...
class CursorDemo(tk.Frame):
    def __init__(self, master=None):
        ...
        for i, cursor in enumerate(cursors):
            tk.Label(self, cursor=cursor, text=cursor, bg=choice(colors), relief=tk.RAISED, width=15, pady=15).grid(row=(i // 8), column=(i % 8))
        tk.Label(self, cursor='none', text='No Cursor', bg=choice(colors), relief=tk.RAISED, pady=15).grid(row=(len(cursors) // 8 + 1), columnspan=8, sticky="ew")

        '''# This is where the custom cursor files are used
        try:
            tk.Label(self, cursor='@nat1059.cur',
                     text='Starfruit (totally custom, downloaded from http://www.cursors-4u.com)', relief=tk.RAISED,
                     padx=15, pady=15).grid(row=(len(cursors) // 8 + 2), columnspan=8, sticky="ew")
            tk.Label(self, cursor='@gam1537x.cur',
                     text='Diamond Sword (totally custom, downloaded from http://www.cursors-4u.com)', relief=tk.RAISED,
                     padx=15, pady=15).grid(row=(len(cursors) // 8 + 3), columnspan=8, sticky="ew")
        except tk.TclError:
